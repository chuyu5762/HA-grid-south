...
- ✅ 支持手机号、短信验证码和密码（可选）登录，支持南网在线 APP、微信、支付宝扫码登录
- ✅ 支持多个南网账户（每个账户一个集成），支持单个账户下的多个缴费号
- ✅ 数据自动抓取和更新（默认间隔 4 小时，可配置）
- ✅ 全程 GUI 配置，无需编辑 yaml（暂不支持 yaml 配置）
...
- 本年度账单用电量、电费（已结算月份，更新到上个月）
- 本年度实际用电量、电费（账单 + 当月）
- 本年度每月用电量、电费（非实时，更新到上个月）
- 上年度总用电量、总电费
- 上年度每月用电量、电费
- 当月累计用电量、电费（非实时，有 2 天左右的延迟）
- 当月每日用电量、电费（非实时，有 2 天左右的延迟）
- 上月累计用电量、电费
- 上月每日用电量、电费

❌ **不支持**阶梯电费设置（仅能获取当前所在阶梯）、峰谷电价设置和电费计算（本插件只进行数据抓取和转换，不进行任何计算），暂时也没有支持计划（南网暂时没有统一的 API），如有需求，建议单独创建对应的电价实体。

❌ 因南网登录 API 调整，登录态失效后无法自动重新登录，需要手动重新登录；集成会自动保留最近一次有效数据，并降低刷新频率等待重新认证。
...
1. 下载本仓库的 [最新代码](https://github.com/chuyu5762/HA-grid-south/archive/refs/heads/master.zip)
...
A: 这是因为南网 API 调整，登录态会过期。请重新登录（集成内可发起重新认证，登录后会自动更新原有配置）。

### Q: API 响应解析错误 (JSONDecodeError)

A: 本仓库已修复此问题。如果你使用的是最上游原仓库 `CubicPill/china_southern_power_grid_stat`，请切换到本仓库。
